...
(function(process,
  ...
  function startup() {
    ...
    const browserGlobals = !process._noBrowserGlobals || process.isCitizenAltMode;
    if (browserGlobals) {
      setupGlobalTimeouts();
      setupGlobalConsole();
    }
    ...
    if (NativeModule.exists('_third_party_main') && !process.isCitizenAltMode) {
  ...
  function setupInspector(originalConsole, wrappedConsole, Module) {
    if (!process.config.variables || !process.config.variables.v8_enable_inspector) {
      return;
    }
